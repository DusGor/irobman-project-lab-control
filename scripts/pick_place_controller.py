...
class PickPlaceController:
    def __init__(self, cube_name: str) -> None:
        ...
        self.gripper_cmd_pub = rospy.Publisher(
            "/panda_gripper/cmd", data_class=std_msgs.msg.String, queue_size=10
        )
    ...
    def _pick(self) -> bool:
        ...
        if (cube_pose := self._get_cube_poses()[0]) is not None:
            ...
            quaternion = tf.transformations.quaternion_from_euler(
                0, math.pi, cube_rpy[2] - 0 * math.pi / 4
            )  # Orientation has to be pitch=pi to point downwards and cube yaw - pi/4
            ...
            grasp_pose.position.z = (
                cube_pose.position.z + self.cube_size + 0.058
            )  # 0.058 is half the distance between link 8 and the end of the EE
    ...
    def _place(self) -> bool:
        ...
        if (cube_pose := self._get_cube_poses()[0]) is not None:
            ...
            place_location.pre_place_approach.direction.header.frame_id = (
            ...
            place_location.post_place_retreat.direction.header.frame_id = (
                self._planning_frame
            )
            # Direction is set as positive z axis
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            cube_pose.position.z = 0.2402

            # cube_pose.orientation.w = 0.000
            # cube_pose.orientation.x = -0.9124
            # cube_pose.orientation.y = 0.4092
            # cube_pose.orientation.z = -0.0047
            ...
            if self.cube_pose:
                ...
                pose.position.z = self.cube_pose.position.z + 0.2

                cube_quat = [
                    self.cube_pose.orientation.x,
                    self.cube_pose.orientation.y,
                    self.cube_pose.orientation.z,
                    self.cube_pose.orientation.w,
                ]
                cube_rpy = tf.transformations.euler_from_quaternion(cube_quat)
